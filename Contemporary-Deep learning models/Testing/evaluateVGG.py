...
classifier=load_model('ear_vgg16.h5py')
#classifier = VGG19('vgg19_weights.h5')
feature = classifier.get_layer('fc1').output
classifier.summary()
...
images = [[0 for x in range(3)] for y in range(114)]

path='UND_E_Swapped_test'
s=os.listdir(path)
s.sort()
...
for i in s :
    ...
    for p in range(1,poses+1):
        print(path+'/' +str(i)+'/'+str(p) + '.jpg')
        current_pose = cv2.imread(path+'/'+str(i)+'/'+str(p)+'.jpg')
