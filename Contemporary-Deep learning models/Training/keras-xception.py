...
def load_data(path):
    ...
    for sub in os.listdir(path):
        for img in os.listdir(os.path.join(path, sub)):
            ...
            print(pose)
            pose = cv2.resize(pose, (299, 299))
            pose = xception.preprocess_input(pose)
            X_train.append(pose)
            print(len(X_train))
...
model_vgg16 = xception.Xception(input_shape=(299, 299, 3),weights= None,  include_top=True)

x = Dense(405, kernel_initializer='glorot_normal', name='imagenet')(model_vgg16.layers[-2].output)
...
'''
#ear_vgg16=load_model('inception.h5')
...
history = ear_vgg16.fit(X_train, Y_train, batch_size=12, epochs=200, verbose=1, shuffle=True)

#ear_vgg16.save_weights('ear_vgg16.h5py')
ear_vgg16.save('./Xception_Results/ear_Xception.h5py')
...
np.savetxt('Xception_Results/Loss.txt',loss1)
np.savetxt('Xception_Results/Val_Loss.txt',acc1)
