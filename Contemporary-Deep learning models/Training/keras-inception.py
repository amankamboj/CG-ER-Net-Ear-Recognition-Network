...
def load_data(path):
    ...
    for sub in os.listdir(path):
        for img in os.listdir(os.path.join(path, sub)):
            ...
            pose = inception_v3.preprocess_input(pose)
            X_train.append(pose)
            print(len(X_train))
...
model_vgg16 = inception_v3.InceptionV3(input_shape=(299, 299, 3),weights= None,  include_top=True)
...
ear_vgg16.save('./Inception_Results/ear_inception.h5py')
...
np.savetxt('Inception_Results/Loss.txt',loss1)
np.savetxt('Inception_Results/Val_Loss.txt',acc1)
