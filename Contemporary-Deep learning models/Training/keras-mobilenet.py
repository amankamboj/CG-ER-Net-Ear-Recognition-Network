...
def load_data(path):
    ...
    for sub in os.listdir(path):
        for img in os.listdir(os.path.join(path, sub)):
            ...
            pose = mobilenet.preprocess_input(pose)
            X_train.append(pose)
            print(len(X_train))
...
model_vgg16 = mobilenet.MobileNet(input_shape=(224, 224, 3),weights=None,  include_top=True)
...
ear_vgg16.save('./Mobilenet_Results/ear_mobilenet.h5py')
...
np.savetxt('Mobilenet_Results/Loss.txt',loss1)
np.savetxt('Mobilenet_Results/Val_Loss.txt',acc1)
