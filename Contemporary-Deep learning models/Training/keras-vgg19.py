...
def load_data(path):
    ...
    for sub in os.listdir(path):
        for img in os.listdir(os.path.join(path, sub)):
            ...
            pose = vgg16.preprocess_input(pose)
            X_train.append(pose)
            print(len(X_train))
...
model_vgg16 = vgg16.VGG16(input_shape=(224, 224, 3), weights=None, include_top=True)
...
ear_vgg16.save('./VGG19_Results/ear_vgg19.h5py')
...
np.savetxt('VGG19_Results/Loss.txt',loss1)
np.savetxt('VGG19_Results/Val_Loss.txt',acc1)
